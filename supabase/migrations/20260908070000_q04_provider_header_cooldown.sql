-- Q04: provider headers may reduce shared capacity, never create capacity.
BEGIN;

ALTER TABLE ops.api_football_budget_state
    ADD COLUMN provider_daily_exhausted boolean NOT NULL DEFAULT false;
...
BEGIN
    ...
    IF state.minute_window <> minute_at THEN
        ...
        SELECT * INTO state FROM ops.api_football_budget_state WHERE singleton;
    END IF;
    -- API-Football does not document a daily-reset header.  A provider's
    -- explicit zero remaining therefore remains blocked until a privileged
    -- operator reconciles it; UTC rollover must not invent provider capacity.
    IF state.provider_daily_exhausted THEN
    ...
    UPDATE ops.api_football_budget_state SET daily_used=daily_used+1, minute_used=minute_used+1,
      ...
      legacy_manual_used=legacy_manual_used + CASE WHEN p_consumer='legacy_manual' THEN 1 ELSE 0 END WHERE singleton;
...
BEGIN
    ...
    -- Only exact non-negative integer strings are considered.  Contradictory,
    -- stale, missing and non-string values are ignored rather than credited.
    minute_limit := CASE WHEN headers->>'x-ratelimit-limit' ~ '^[0-9]+$' THEN (headers->>'x-ratelimit-limit')::integer END;
    minute_remaining := CASE WHEN headers->>'x-ratelimit-remaining' ~ '^[0-9]+$' THEN (headers->>'x-ratelimit-remaining')::integer END;
    daily_limit := CASE WHEN headers->>'x-ratelimit-requests-limit' ~ '^[0-9]+$' THEN (headers->>'x-ratelimit-requests-limit')::integer END;
    daily_remaining := CASE WHEN headers->>'x-ratelimit-requests-remaining' ~ '^[0-9]+$' THEN (headers->>'x-ratelimit-requests-remaining')::integer END;
    IF (minute_limit IS NOT NULL AND minute_remaining IS NOT NULL AND minute_remaining > minute_limit)
       OR (daily_limit IS NOT NULL AND daily_remaining IS NOT NULL AND daily_remaining > daily_limit) THEN
        RETURN;
    END IF;
    ...
    IF p_status_code = 429 OR minute_remaining = 0 THEN
        IF coalesce(headers->>'retry-after', '') ~ '^[0-9]+$' THEN
    ...
    IF daily_remaining = 0 THEN
        UPDATE ops.api_football_budget_state SET provider_daily_exhausted=true WHERE singleton;
    END IF;
END;
$$;
