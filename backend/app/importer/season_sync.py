...
class SeasonalRunLeaseLost(SeasonalSyncError):
    """Another worker took the run lease while this process was active."""
...
class SeasonalSyncRepository(Protocol):
    ...
    def finish_run(
        self, run_id: int, run_token: int, *, status: str, checkpoint: Mapping[str, Any]
    ) -> None: ...
...
class PostgresSeasonalSyncRepository:
    ...
    def _acquire_new_run(self, policies: Sequence[SeasonalLeaguePolicy]) -> tuple[int, int]:
        row = self._conn.execute(
            """INSERT INTO ops.sync_runs(
                   provider_id,operation,scope,status,started_at,
                   lease_owner,lease_token,lease_expires_at
               ) VALUES(%s,%s,%s,'running',clock_timestamp(),%s,nextval('ops.sync_run_lease_token_seq'),
                   clock_timestamp() + (%s::interval))
               RETURNING id, lease_token""",
            (self._provider, OPERATION, Jsonb({"policy_version": POLICY_VERSION}), self._lease_owner, f"{RUN_LEASE_SECONDS} seconds"),
        ).fetchone()
            ...
            if prior is not None:
                ...
                row = self._conn.execute(
                    """UPDATE ops.sync_runs SET status='running',
                       lease_owner=%s,
                       lease_token=nextval('ops.sync_run_lease_token_seq'),
    ...
    def renew_run_lease(self, run_id: int, run_token: int) -> bool:
        row = self._conn.execute(
            """UPDATE ops.sync_runs
               SET lease_expires_at=clock_timestamp() + (%s::interval)
               WHERE id=%s AND status='running' AND lease_owner=%s AND lease_token=%s
                 AND lease_expires_at >= clock_timestamp()
               RETURNING true""",
            (f"{RUN_LEASE_SECONDS} seconds", run_id, self._lease_owner, run_token),
        ).fetchone()
        return row is not None

    def _lock_owned_run(self, run_id: int, run_token: int) -> None:
        row = self._conn.execute(
            "SELECT id FROM ops.sync_runs WHERE id=%s FOR UPDATE",
            (run_id,),
        ).fetchone()
        if row is None:
        ...
        current = self._conn.execute(
            """SELECT 1 FROM ops.sync_runs
               WHERE id=%s AND status='running' AND lease_owner=%s AND lease_token=%s
                 AND lease_expires_at > clock_timestamp()""",
            (run_id, self._lease_owner, run_token),
        ).fetchone()
        if current is None:
            raise SeasonalRunLeaseLost("run lease was lost to another worker")

    def _lock_owned_run_and_item(self, item: SeasonalWorkItem, run_token: int) -> None:
        # Lock first and validate second: a lock acquisition can wait long
        # enough for either lease to expire.  The final SQL check therefore
        # runs only after both run -> item locks are held.
        run = self._conn.execute(
            """SELECT run.id FROM ops.sync_runs run
               WHERE run.id=(SELECT item.run_id FROM ops.sync_work_items item WHERE item.id=%s)
               FOR UPDATE""",
            (item.id,),
        ).fetchone()
        if run is None:
        ...
        current = self._conn.execute(
            """SELECT 1
               ...
               WHERE run.id=%s AND item.id=%s
                 ...
                 AND item.run_lease_token=%s AND item.lease_expires_at > clock_timestamp()
                 AND item.job_type='legacy'""",
            (int(run[0]), item.id, self._lease_owner, run_token, self._lease_owner, run_token),
        ).fetchone()
        if current is None:
            raise SeasonalRunLeaseLost("seasonal work item lease was lost before applying its result")
    ...
    def claim_next(self, run_id: int, run_token: int, policies: Mapping[int, SeasonalLeaguePolicy]) -> SeasonalWorkItem | None:
        ...
        league_external_id = scope.get("league_external_id") if isinstance(scope, Mapping) else None
    ...
    def finish_run(self, run_id: int, run_token: int, *, status: str, checkpoint: Mapping[str, Any]) -> None:
        with self._conn.transaction():
            ...
            if status == "succeeded":
                ...
                # Locking every item can itself wait; recheck the run lease at
                # the final write boundary after that wait.
                self._lock_owned_run(run_id, run_token)
            self._conn.execute(
                """UPDATE ops.sync_runs
...
class SeasonalSyncWorker:
    ...
    async def _fetch(self, request: BaseRequest) -> CollectedBaseResponse:
        ...
        started = _now()
        try:
            response = await self._run_with_lease_heartbeat(
                self._provider.get(request.endpoint, params=request.params)
            )
    ...
    async def _run_with_lease_heartbeat(self, request: asyncio.Task[Any] | asyncio.Future[Any]) -> Any:
        if self._run_id is None or self._run_token is None:
            return await request

        stop = asyncio.Event()
        interval = max(1, RUN_LEASE_SECONDS // 4)
        heartbeat_task = asyncio.create_task(self._renew_run_lease_while_running(stop, self._run_id, self._run_token))
        try:
    ...
    async def _renew_run_lease_while_running(self, stop: asyncio.Event, run_id: int, run_token: int) -> None:
        while True:
            await asyncio.sleep(max(1, RUN_LEASE_SECONDS // 4))
            if stop.is_set():
                return
            if not self._repository.renew_run_lease(run_id, run_token):
                raise SeasonalRunLeaseLost("run lease was lost during long request")

    async def _process(self, item: SeasonalWorkItem) -> SeasonalLeagueReport:
        run_token = self._run_token
        if run_token is None:
            raise SeasonalSyncError("run token is not available for processing")
        self._assert_run_lease()
        policy = item.policy
    ...
    async def run_once(self) -> SeasonalSyncReport:
        ...
        run_token = acquisition.run_token
        self._run_id = run_id
        ...
        try:
            self._assert_run_lease()
            while (item := self._repository.claim_next(run_id, run_token, self._policy_by_league)) is not None:
                try:
                    self._assert_run_lease()
                    reports.append(await self._process(item))
                except ProviderQuotaExhausted as error:
                    terminal_status = "failed"
                    self._repository.fail(item, run_token, checkpoint={"outcome": "quota_stopped"}, error=type(error).__name__)
                    raise
                except APIFootballBudgetError as error:
                    self._repository.defer(
                        item,
                        run_token,
                        checkpoint={"outcome": "budget_pending", "reason": type(error).__name__},
                        error=type(error).__name__,
                        delay_seconds=budget_retry_delay_seconds(error),
                    )
                    ...
                    terminal_status = "failed"
                    break
                except SeasonalRunLeaseLost:
                    raise
                except (APIFootballHTTPError, APIFootballAPIError, ActiveSeasonImportError, SeasonalSyncError) as error:
                    terminal_status = "failed"
                    self._repository.fail(item, run_token, checkpoint={"outcome": "failed"}, error=type(error).__name__)
            ...
            if not lost_ownership:
                ...
                try:
                    self._repository.finish_run(run_id, run_token, status=terminal_status, checkpoint=checkpoint)
                except SeasonalRunLeaseLost:
                    lost_ownership = True
                    terminal_status = "running"
            self._run_id = None
    ...
    def _assert_run_lease(self) -> None:
        if self._run_id is None or self._run_token is None:
            return
        if not self._repository.renew_run_lease(self._run_id, self._run_token):
            raise SeasonalRunLeaseLost("run lease was lost to another worker")
