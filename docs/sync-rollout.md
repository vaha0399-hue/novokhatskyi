...
для одного запуска и не для совокупности всех пилотов. P04 намеренно не
запускает preflight, поэтому точное число `N_missing` неподтверждённых/неполных
пар неизвестно.

| Сценарий | Расчёт | Бюджет |
| --- | --- | ---: |
| Реально отсутствующая статистика | Discovery/fixtures/standings: `3 + 5 + 1 = 9`; затем только `min(N_missing, 90)` статистических запросов в одном run | `9 + min(N_missing, 90)`; верхняя граница одного run — **99** |
| Гипотетическая повторная загрузка всех 948 fixtures | `9` метаданных + `948` одиночных statistics-запросов; подтверждённые пары в обычном backfill так не перезагружаются | **957** без повторов; это не план запуска |

Число `948` больше не является бюджетом реальной истории: при подтверждённых
парах оно расходует ноль запросов. Ни один из сценариев не запускается до Q04,
когда общий лимит станет атомарно контролируемым.
...
| --- | ---: | --- |
| Операционные jobs | 2,000 | Calendar, live, results, statistics retries и corrections из таблицы выше. |
| История | 90 | Не больше run-cap одного historical backfill; расходуется только на `N_missing`. |
