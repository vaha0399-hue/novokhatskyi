...
`second` supports the P04 25-second cadence. It also has priority, history
depth, an immutable per-insert instance ID, a monotonic version, and an
optional timed pause.
...
callback.  The second call rereads the policy and requires the enqueue instance
ID and version, so a changed, deleted/recreated, disabled, or paused policy
cannot authorize stale work. Ordinary work requires its own coverage to be `covered`;
`coverage_refresh` is the explicit allowed freshness-check exception. Unknown
